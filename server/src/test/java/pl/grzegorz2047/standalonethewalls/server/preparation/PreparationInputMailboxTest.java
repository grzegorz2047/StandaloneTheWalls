...
class PreparationInputMailboxTest {
    ...
    void keepsOnlyTheNewestMonotonicInputForTheOpenRound() {
        ...
        mailbox.open(3L);
        PreparationInput first = new PreparationInput(3L, 1L, 127, 0, false, 0, 0);
        PreparationInput newest = new PreparationInput(3L, 2L, 0, -127, false, 9_000, -500);
    ...
    void rejectsClosedWrongRoundAndNonMonotonicInputWithoutGrowingState() {
        PreparationInputMailbox mailbox = new PreparationInputMailbox();
        PreparationInput input = new PreparationInput(1L, 1L, 0, 0, false, 0, 0);
        ...
        assertThat(mailbox.offer(new PreparationInput(2L, 2L, 0, 0, false, 0, 0)))
                .isEqualTo(PreparationInputMailbox.OfferResult.ACCEPTED);
        assertThat(mailbox.offer(new PreparationInput(2L, 2L, 127, 0, false, 0, 0)))
                .isEqualTo(PreparationInputMailbox.OfferResult.NON_MONOTONIC_SEQUENCE);
        assertThat(mailbox.offer(new PreparationInput(2L, 1L, 127, 0, false, 0, 0)))
                .isEqualTo(PreparationInputMailbox.OfferResult.NON_MONOTONIC_SEQUENCE);
        assertThat(mailbox.drainLatest()).contains(new PreparationInput(2L, 2L, 0, 0, false, 0, 0));

        mailbox.close();
        assertThat(mailbox.offer(new PreparationInput(2L, 3L, 0, 0, false, 0, 0)))
