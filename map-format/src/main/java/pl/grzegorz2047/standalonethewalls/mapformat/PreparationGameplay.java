...
public record PreparationGameplay(
        int schemaVersion,
        List<PreparationRegion> regions,
        List<PreparationMapSpawn> spawns) {
    ...
    public PreparationGameplay {
        ...
        if (regions.size() != 2 && regions.size() != 4) {
            throw new IllegalArgumentException("preparation gameplay requires exactly 2 or 4 regions");
        }
        if (spawns.size() < regions.size() || spawns.size() > MAXIMUM_SPAWNS) {
            throw new IllegalArgumentException("preparation spawn count is outside the supported range");
        }

        Map<PreparationTeam, PreparationRegion> regionsByTeam = new EnumMap<>(PreparationTeam.class);
        ...
        for (int left = 0; left < regions.size(); left++) {
            for (int right = left + 1; right < regions.size(); right++) {
                if (regions.get(left).overlapsVolume(regions.get(right))) {
                    throw new IllegalArgumentException("preparation regions cannot overlap in volume");
                }
            }
        }
        ...
        for (PreparationMapSpawn spawn : spawns) {
            ...
            if (!region.contains(candidate.position())) {
                throw new IllegalArgumentException("preparation spawn must be inside its team region");
            }
