package pl.grzegorz2047.standalonethewalls.transport.bctls;

import java.io.IOException;
import java.net.Socket;
import java.security.SecureRandom;
import java.util.Objects;
import org.bouncycastle.tls.TlsClientProtocol;
import org.bouncycastle.tls.crypto.impl.jcajce.JcaTlsCrypto;

/** Performs one blocking, pinned TLS 1.3 client handshake over an existing socket. */
public final class Tls13ClientConnector {
    private Tls13ClientConnector() {
        throw new AssertionError("No instances");
    }

    public static Tls13Connection connect(
            Socket socket, PinnedServerTrustManager trustManager, SecureRandom secureRandom)
            throws IOException, TlsTransportException {
        Tls13SocketPolicy.validate(socket);
        Objects.requireNonNull(trustManager, "trustManager");
        Objects.requireNonNull(secureRandom, "secureRandom");

        JcaTlsCrypto crypto;
        try {
            crypto = BouncyCastleTlsCryptoFactory.create(secureRandom);
        } catch (RuntimeException exception) {
            closeWithSuppressed(socket, exception);
            throw new TlsTransportException(
                    TlsTransportException.Code.CRYPTO_INITIALIZATION_FAILED,
                    "unable to initialize Bouncy Castle TLS cryptography",
                    exception);
        }

        SunderfrontTlsClient client = new SunderfrontTlsClient(crypto, trustManager);
        TlsClientProtocol protocol =
                new TlsClientProtocol(socket.getInputStream(), socket.getOutputStream());
        try {
            protocol.connect(client);
            return new Tls13Connection(socket, protocol, client.security());
        } catch (IOException | RuntimeException exception) {
            closeWithSuppressed(socket, exception);
            throw exception;
        }
    }

    private static void closeWithSuppressed(Socket socket, Throwable failure) {
        try {
            socket.close();
        } catch (IOException closeFailure) {
            failure.addSuppressed(closeFailure);
        }
    }
}
