# Windows first-run reliability alpha release procedure

The release workflow in this source version handles only `v0.1.0-alpha.4`. The
build reads `release/version.txt`; it never discovers or substitutes a moving
`latest` value. Published alpha.1 through alpha.3 tags and assets remain immutable
and must not be replaced.

## Repository protection required once

Create a GitHub tag ruleset for `v0.1.0-alpha.4` that permits creation by the
...
2. Wait for the push CI on `main`, including `verify-windows-main`, to finish
   ...
   will be built, tested, tagged, and published.
3. Create branch `publish-v0.1.0-alpha.4` directly from that SHA.
4. Add exactly one file, `.release-trigger/v0.1.0-alpha.4.json`, in exactly one
...
6. When an automation environment can write repository commits but its push does
   ...
   `publish-v0.1.0-alpha.4` to `main`, review the one-file diff, and mark it ready
...
A push to `publish-v0.1.0-alpha.4`, or marking its reviewed same-repository trigger
...
5. run the complete Java 21 quality gate on Windows and execute the JVM first-run
   launchers, including fresh, repeated-complete, partial, and mixed-package
   server credential paths;
...
10. run `gh release create --target <candidate SHA>`, which creates
    `v0.1.0-alpha.4` and the prerelease only after every gate succeeds;
11. verify that the tag resolves to the candidate and the Release contains exactly:
    - `sunderfront-client-windows-x64-0.1.0-alpha.4.zip`;
    - `sunderfront-client-0.1.0-alpha.4.zip`;
    - `sunderfront-server-0.1.0-alpha.4.zip`;
    - `SHA256SUMS`.
...
```powershell
Get-FileHash .\sunderfront-client-windows-x64-0.1.0-alpha.4.zip -Algorithm SHA256
Get-FileHash .\sunderfront-client-0.1.0-alpha.4.zip -Algorithm SHA256
Get-FileHash .\sunderfront-server-0.1.0-alpha.4.zip -Algorithm SHA256
```
...
Never reuse `v0.1.0-alpha.4` for different bytes.
