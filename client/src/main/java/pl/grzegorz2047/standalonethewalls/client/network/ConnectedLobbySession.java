...
public final class ConnectedLobbySession implements AutoCloseable {
    ...
    ConnectedLobbySession(
        ...
        requireSelf(initialSnapshot, playerId, handle);
        receiver =
                Thread.ofVirtual()
    ...
    private void finish(Optional<DirectConnectFailure> failure) {
        ...
        closeStage.whenComplete(
                (ignored, closeFailure) -> completeClose(failure, closeFailure));
    }
    ...
    private void completeClose(Optional<DirectConnectFailure> failure, Throwable closeFailure) {
        try {
            ...
            // Ownership cleanup must not expose internal callback failures to presentation code.
        }
    ...
    private static void requireSelf(
            LobbySnapshot snapshot, PlayerId playerId, CanonicalHandle handle) {
        if (!containsExactSelf(snapshot, playerId, handle)) {
    ...
    static boolean containsExactSelf(
            LobbySnapshot snapshot, PlayerId playerId, CanonicalHandle handle) {
        for (LobbyMember member : snapshot.members()) {
            if (member.playerId().equals(playerId)) {
                return member.handle().equals(handle);
            }
        }
        return false;
    }
