...
public final class PreparationMovementProtocolCodec {
    ...
    public static final int PLAYER_SNAPSHOT_BYTES = 85;
    ...
    private static final int INPUT_SCHEMA_VERSION = 4;
    ...
    private static final int INPUT_FLAG_JUMP = 1 << 2;
    private static final int KNOWN_INPUT_FLAGS =
    ...
    public static byte[] encodeSnapshot(PreparationWorldSnapshot snapshot) {
        ...
        for (PreparationPlayerSnapshot player : value.players()) {
            ...
            output.put(playerId)
                    ...
                    .put((byte) snapshotFlags(player))
                    .putInt(player.verticalVelocityMillimetresPerSecond())
    ...
    public static PreparationWorldSnapshot decodeSnapshot(byte[] payload)
        ...
        for (int index = 0; index < playerCount; index++) {
            ...
            SnapshotFlags flags = requireSnapshotFlags(input.get());
            int verticalVelocityMillimetresPerSecond = requireVerticalVelocity(input.getInt());
            if (flags.grounded() && verticalVelocityMillimetresPerSecond != 0) {
                throw failure(
                        PreparationProtocolException.Code.INVALID_STATE,
                        "grounded preparation snapshot has non-zero vertical velocity");
            }
    ...
    private static int snapshotFlags(PreparationPlayerSnapshot player) {
        int flags = 0;
        if (player.crouching()) {
            flags |= SNAPSHOT_FLAG_CROUCH;
        }
        if (player.grounded()) {
    ...
    private static InputFlags requireInputFlags(byte raw) throws PreparationProtocolException {
        ...
        boolean sprinting = (flags & INPUT_FLAG_SPRINT) != 0;
        boolean crouching = (flags & INPUT_FLAG_CROUCH) != 0;
        boolean jumping = (flags & INPUT_FLAG_JUMP) != 0;
        ...
        return new InputFlags(sprinting, crouching, jumping);
    }

    private static SnapshotFlags requireSnapshotFlags(byte raw)
            throws PreparationProtocolException {
        int flags = Byte.toUnsignedInt(raw);
    ...
    private static int requireVerticalVelocity(int value) throws PreparationProtocolException {
        if (value < PreparationVerticalMotion.MINIMUM_VERTICAL_VELOCITY_MILLIMETRES_PER_SECOND
                || value
                        > PreparationVerticalMotion
                                .MAXIMUM_VERTICAL_VELOCITY_MILLIMETRES_PER_SECOND) {
            throw failure(
                    PreparationProtocolException.Code.INVALID_STATE,
                    "preparation vertical velocity is invalid");
        }
        return value;
    }
    ...
    private record InputFlags(boolean sprinting, boolean crouching, boolean jumping) {}

    private record SnapshotFlags(boolean crouching, boolean grounded) {}
}
